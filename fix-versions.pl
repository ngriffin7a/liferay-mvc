...
# of the liferay-mvc project. It parses the project version information from
...
# Primitives
#
my($liferayMVCVersion,$liferayMVCVersionShort,$liferayMVCVersionShortMajor1DotMajor2,$major1,$major2,$minor);
my($portalVersion,$portalVersions,$portalDtdDisplay,$portalDtdUrl,$liferayMVCVersionWithoutSnapshot);
my($mvcVersion,$mvcVersionURL,$mvcMajor,$mvcMinor);
my($liferayMVCMajor1,$liferayMVCMajor2,$liferayMVCMinor,);
...
while(<POM>) {
	...
	if (/artifactId>liferay-mvc</) {

		$_ = <POM>;
		/<version>(.*)</;
		$liferayMVCVersion = $1;
		print "liferayMVCVersion = $liferayMVCVersion\n";

		$_ = $liferayMVCVersion;
		s/-SNAPSHOT//;
		$liferayMVCVersionWithoutSnapshot = $_;
		print "liferayMVCVersionWithoutSnapshot = $liferayMVCVersionWithoutSnapshot\n";

		$_ = $liferayMVCVersionWithoutSnapshot;
		s/-(.*)//;
		$liferayMVCVersionShort = $_;
		print "liferayMVCVersionShort = $liferayMVCVersionShort\n";

		($liferayMVCMajor1,$liferayMVCMajor2,$liferayMVCMinor) = split /\./;
		$liferayMVCVersionShortMajor1DotMajor2 = "${liferayMVCMajor1}.${liferayMVCMajor2}";
		print "liferayMVCVersionShortMajor1DotMajor2 = $liferayMVCVersionShortMajor1DotMajor2\n";
	...
	if ($file eq "pom.xml" and $File::Find::name ne "./pom.xml" and $File::Find::name !~ /\/target/) {
		...
		while(<IN>) {
			...
			if ($inParent == 1 and /<version/) {
				s/ersion>..*</ersion>$liferayMVCVersion</;
				$inParent = 0;
			}
			print OUT;
		}
